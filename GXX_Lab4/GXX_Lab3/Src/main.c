...
static void MX_DMA1_DMA_Init(void);

char Temp[30];

//also used for DMA part
int UART_print_string (UART_HandleTypeDef* huart1, char* array, int length)
{
	if (HAL_UART_Transmit_DMA(huart1, (uint8_t*)array, length) == HAL_OK){
		return 1;
	}else
		return 0;
}
...
//		}
			...
			if(HAL_ADC_PollForConversion(&hadc1, 10000) == HAL_OK){	//checks if conversion is done
				...
				if(arrayCount<30){		//until 10 values are obtained
					...
					tempCelcius = __HAL_ADC_CALC_TEMPERATURE(3300, tempVoltage, ADC_RESOLUTION_12B);
					Temp[arrayCount] = (tempCelcius/10) + '0';
					Temp[arrayCount+1] = (tempCelcius%10) + '0';
					Temp[arrayCount+2] = '\n';
					arrayCount = arrayCount + 3;
				}else{
					arrayCount = 0; //reset arrayPointer
					UART_print_string(&huart1, Temp, 30);						
				}	
			}			//VREF = 3.3 
...
void SystemClock_Config(void)
{
  ...
  if (HAL_PWREx_ControlVoltageScaling(PWR_REGULATOR_VOLTAGE_SCALE1) != HAL_OK)
  {
    ...
    */
  HAL_SYSTICK_Config(HAL_RCC_GetHCLKFreq()/10);//HAL_RCC_GetHCLKFreq()
...
static void MX_USART1_UART_Init(void) //hdma_usart1_tx
{
